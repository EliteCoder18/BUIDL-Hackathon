...
def score_risk(f):
 v=np.asarray([float(f[n]) for n in FEATURES]);span=np.maximum(BUNDLE["maximum"]-BUNDLE["minimum"],1e-9);drift=float(np.max(np.maximum(BUNDLE["minimum"]-v,0)/span+np.maximum(v-BUNDLE["maximum"],0)/span));reasons=[];warnings=list(EVALUATION.warnings)
 # Agent identity is metadata, not a trained feature. New ERC-8004 agents are a
 # normal cold-start case, so disclose them without pretending they are OOD.
 if f.get("agent_id","agent-00") not in KNOWN_AGENTS: warnings.append("unseen-agent-identity")
