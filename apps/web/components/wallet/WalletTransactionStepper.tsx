...
export function WalletTransactionStepper({ steps, busy, error, onAdvance, retry }: { steps: WalletStepView[]; busy: boolean; error?: string; onAdvance: () => void; retry?: { label: string; onClick: () => void } }) {
  ...
  return <section className="wallet-stepper" aria-live="polite" aria-busy={busy}>
    ...
    {active && <button type="button" className={`technical-button technical-button--primary technical-button--wide${busy ? " technical-button--pending" : ""}`} disabled={busy} onClick={onAdvance}>{busy ? <><i className="button-waiting-dot" />AWAITING METAMASK CONFIRMATION…</> : `CONTINUE IN METAMASK · ${active.chain}`}</button>}
