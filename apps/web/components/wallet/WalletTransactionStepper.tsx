...
export function WalletTransactionStepper({ steps, busy, error, onAdvance, retry, cancel }: { steps: WalletStepView[]; busy: boolean; error?: string; onAdvance: () => void; retry?: { label: string; onClick: () => void }; cancel?: { label: string; onClick: () => void } }) {
  ...
  return <section className="wallet-stepper" aria-live="polite" aria-busy={busy}>
    ...
    {active && <div className="wallet-stepper__actions">
      <button type="button" className={`technical-button technical-button--primary technical-button--wide${busy ? " technical-button--pending" : ""}`} disabled={busy} onClick={onAdvance}>{busy ? <><i className="button-waiting-dot" />AWAITING METAMASK CONFIRMATION…</> : `CONTINUE IN METAMASK · ${active.chain}`}</button>
      {cancel && <button type="button" className="technical-button wallet-stepper__cancel" disabled={busy} onClick={cancel.onClick}>{cancel.label}</button>}
    </div>}
