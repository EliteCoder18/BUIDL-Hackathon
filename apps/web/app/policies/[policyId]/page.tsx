...
export default function PolicyPage({ params }: { params: { policyId: string } }) {
  ...
  return <div className="route-stack">
    ...
    <div className="dashboard-grid dashboard-grid--policy"><TechnicalPanel eyebrow="CAPITAL BOND" title={`${money(policy?.coverageAmount)} coverage`}><LossWaterfall coverage={Number(policy?.coverageAmount ?? 0) / 1e6} loss={Number(policy?.clientPayout ?? 0) / 1e6} state={state} /><div className="integrity-grid"><span><small>UNDERWRITER</small><code>{short(policy?.underwriter)}</code></span><span><small>PREMIUM</small><strong>{money(policy?.premiumAmount)}</strong></span><span><small>JUNIOR FIRST LOSS</small><strong>{money(policy?.juniorAmount)}</strong></span><span><small>SENIOR LOCK</small><strong>{money(policy?.seniorAmount)}</strong></span></div></TechnicalPanel>
    <TechnicalPanel eyebrow="COMMAND SEQUENCER" title="Advance objective lifecycle"><div className="command-stack"><button className="technical-button" disabled={!policy || executed || Boolean(busy)} onClick={() => command("execute", () => api.executeJob(policy!.jobKey, scenario))}>{busy === "execute" ? "MINING EXECUTION…" : `01 / EXECUTE ${scenario.toUpperCase()}`}</button><button className="technical-button" disabled={!executed || Boolean(proof) || Boolean(busy)} onClick={() => command("prove", () => api.proveOutcome(policy!.jobKey))}>{busy === "prove" ? "BUILDING PROOF…" : "02 / BUILD + SUBMIT PROOF"}</button><button className="technical-button technical-button--primary" disabled={!proof || settled || Boolean(busy)} onClick={() => command("settle", () => api.settlePolicy(policyId))}>{busy === "settle" ? "SETTLING CC3 POLICY…" : "03 / SETTLE PERFORMANCE BOND"}</button></div><div className="scenario-toggle"><span>DEMO OUTCOME</span><button className={scenario === "success" ? "active" : ""} disabled={executed} onClick={() => setScenario("success")}>SUCCESS</button><button className={scenario === "violation" ? "active danger" : ""} disabled={executed} onClick={() => setScenario("violation")}>VIOLATION</button></div>{proof && <Link className="text-link" href={`/proofs/${policy!.jobKey}`}>OPEN FULL PROOF EXPLORER →</Link>}</TechnicalPanel></div>
    <TechnicalPanel eyebrow="CROSS-CHAIN VERIFICATION" title="Proof and settlement rail"><ProofRail stages={stages} /></TechnicalPanel>
