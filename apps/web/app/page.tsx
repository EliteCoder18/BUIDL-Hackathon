...
export default function Page() {
  ...
  return (
    <main>
      ...
      <section id="market" className="section"><div className="section-title"><div><p className="eyebrow">Quote auction</p><h2>Junior capital meets pooled liquidity.</h2></div><span className="live">LIVE</span></div><div className="quotes">{marketQuotes.map((quote, index) => <button className={`quote ${selected === index ? "selected" : ""}`} key={quote.name} onClick={() => setSelected(index)}><span>{quote.name}</span><b>{quote.risk}</b><small>failure probability</small><strong>{quote.premium}</strong><small>{quote.tone} premium</small></button>)}</div><div className="policy"><div><p className="eyebrow">Selected policy</p><h3>{selectedQuote.name} guarantees 100 mUSDC treasury mandate</h3><p>20 mUSDC junior first-loss stake · 80 mUSDC senior LP capital · premium split 30/70 on success.</p></div><button className="primary">Accept {selectedQuote.premium} quote</button></div></section>
